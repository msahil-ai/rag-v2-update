...
while True:
    fetched_data = fetch_next_pending_email()

    if not fetched_data:
        print("......No more pending emails. Exiting.")
        break
    email_text, sender_email = fetched_data
...
Never return anything outside JSON.
    ...
    try:
        ...
        extracted_data["sender_email"] = sender_email # Add sender email to the extracted data for later use in RAG
        ...
        extracted_data = {
            ...
            "sender_email": sender_email # Even if extraction fails, we still capture the sender email for debugging and potential follow-up
        }
        print(f"..... JSON extraction failed: {e}")

    # 6. Save to the outputs folder exactly as before
    output_path = os.path.join(output_folder, f"email_{counter}.json")
